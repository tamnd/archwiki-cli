// Package cli builds the archwiki command tree on top of the archwiki library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: archwiki.DefaultConfig()}

	root := &cobra.Command{
		Use:   "archwiki",
		Short: "Search Arch Linux Wiki documentation",
		Long: `archwiki reads Arch Linux Wiki through the public MediaWiki JSON API.
No API key is required. It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

archwiki is an independent tool and is not affiliated with Arch Linux or its maintainers.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = archwiki.NewClient(a.cfg)
	return nil
}
