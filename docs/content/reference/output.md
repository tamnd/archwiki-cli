...
describes what users get. Pick a format with `-o`, or let archwiki choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
archwiki <command> -o table   # aligned columns for reading
archwiki <command> -o jsonl   # one JSON object per line, for piping
archwiki <command> -o json    # a single JSON array
archwiki <command> -o csv     # spreadsheet friendly
archwiki <command> -o tsv     # tab-separated
archwiki <command> -o url     # just the URL column
archwiki <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
archwiki <command> --fields id,title,url
```
...
```bash
archwiki <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
archwiki <command>            # a table, because this is a terminal
archwiki <command> | wc -l    # JSONL, because this is a pipe
```
